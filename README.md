# Pablo Garcia-Cardarelli

Telecommunications engineer and Ph.D. candidate at Tecnun - University of Navarra, working on chipless RFID systems, passive RF sensors, and digital signal processing.  
My research focuses on low-cost, hardware-based sensing for industrial and IoT applications — combining antenna design, circuit and PCB development, and FPGA implementation. I'm especially interested in **wearable systems**, **wireless monitoring**, and **experimental hardware prototyping** — the kind of work where you learn by building and testing.

---

### Areas of Work

- Antenna and tag design for chipless RFID and wireless sensing
- Embedded electronics and PCB development
- Signal processing with Python and MATLAB
- System integration and experimental validation

---

### Experience

- **Tecnun - University of Navarra (Spain)** – Ph.D. research in chipless RFID and sensing systems  
- **Fraunhofer IIS (Germany)** – GNSS signal processing modules on FPGA  
- **MIT Auto-ID Labs (USA)** – Research stay in chipless RFID technologies

---

### Contact

- 📫 [pablogcardarelli@gmail.com](mailto:pablogcardarelli@gmail.com)  
- 🌐 [LinkedIn](https://www.linkedin.com/in/pablogcardarelli)  
- 📄 [Google Scholar](https://scholar.google.com/citations?user=qnEwuk8AAAAJ&hl=es)

---
